...
class CKMeans:
    ...
    def _create_clusters(self, centroids, X, demand, capacities):
        ...
        If adding the each point to the cluster would result.nodes() nx. in surpassing
        ...
        for sample_i, sample in enumerate(X):
            ...
            for i in range(self.k):
                Tind=np.where(sorted_centroids == i)
                ind=Tind[0][0]
                if clusters_filled[ind] + demand[sample_i] <= \
                            capacities[ind]:
                        clusters[ind].append(sample_i)
                        clusters_filled[ind] += demand[sample_i]
                        break
